...
## Последний релиз: 0.7.2

Кратко о недавних изменениях (полная история — в [CHANGELOG.md](CHANGELOG.md)):

- **0.7.2 — исправление ошибок** — единый почтовый ящик теперь учитывает сортировку по убыванию.
- **0.7.1 — исправление ошибок** — уведомления, отправленные как Markdown, теперь отображаются с форматированием (определяется по типу содержимого сообщения).
- **0.7.0 — функциональный релиз** — удаление отдельных уведомлений, фильтрация по приоритету и тегам, поддержка обновлений уведомлений ntfy (обновлённые уведомления заменяют исходное и помечаются как «Изменено»).
